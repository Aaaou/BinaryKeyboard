...
FN 是设备本地控制输入，不依赖 USB、BLE 或 2.4G HID 传输是否已经就绪。核心处理循环必须始终消费 FN 队列，以便设备在未插 USB、无线链路尚未重连或 HID 暂时忙时仍可切换模式、切层和执行本地维护动作。普通键和旋钮属于 HID 输入：当当前传输未就绪时直接丢弃，不能缓存后延迟发送，否则会在重新连接后产生过期按键或卡键。

该边界由 `KBD_Core_Process()` 统一执行：传输未就绪时调用 `DiscardQueuedHidInput()` 清理普通 HID 队列，再调用 `ProcessFnEvents()` 处理 FN 队列；深度唤醒恢复阶段仍按原规则抑制全部输入。此修复同时应用于 BLE/有线、单 2.4G 和三模分支，不改变 HID 报告格式、模式切换命令或接收器协议。
